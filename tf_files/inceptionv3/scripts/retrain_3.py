...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
  ...
  if how_many >= 0:
    ...
    for unused_i in range(how_many):
      ...
      #if class_count==2 and category=='training':
      #  label_index = random.randrange(10)
      #  label_index = int(label_index>6)
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  semi_layer_size  = 1024 
  with tf.name_scope(layer_name):
    with tf.name_scope('weights'):
      #initial_value = tf.truncated_normal(
      #    [bottleneck_tensor_size, class_count], stddev=0.001)
           
      first_initial_value = tf.eye(
          bottleneck_tensor_size, first_layer_size)

      first_layer_weights = tf.Variable(first_initial_value, name='first_layer_weights')
          
      semi_initial_value = tf.eye(
          first_layer_size, semi_layer_size)

      semi_layer_weights = tf.Variable(semi_initial_value, name='semi_final_weights')
      
      initial_value = tf.eye(
          semi_layer_size, class_count)

      layer_weights = tf.Variable(initial_value, name='final_weights')

      variable_summaries(first_layer_weights)
      variable_summaries(semi_layer_weights)
      variable_summaries(layer_weights)
      
    with tf.name_scope('biases'):
      first_layer_biases = tf.Variable(tf.zeros([first_layer_size]), name='first_layer_biases')
      semi_layer_biases = tf.Variable(tf.zeros([semi_layer_size]), name='semi_final_biases')
      layer_biases = tf.Variable(tf.zeros([class_count]), name='final_biases')
      variable_summaries(first_layer_biases)
      variable_summaries(semi_layer_biases)
      variable_summaries(layer_biases)
      
    with tf.name_scope('Wx_plus_b'):
      first_logits = tf.matmul(bottleneck_input, first_layer_weights) + first_layer_biases
      tf.summary.histogram('first_layer_pre_activations', first_logits)
      semi_logits = tf.matmul(first_logits, semi_layer_weights) + semi_layer_biases
      tf.summary.histogram('semi_pre_activations', semi_logits)
      logits = tf.matmul(semi_logits, layer_weights) + layer_biases
      tf.summary.histogram('pre_activations', logits)

  first_layer_tensor = tf.nn.relu(first_logits, name='first_layer_tensor')
  semi_final_tensor = tf.nn.relu(semi_logits, name='semi_final_tensor')
  final_tensor = tf.nn.softmax(logits, name=final_tensor_name)
  tf.summary.histogram('first_layer_activations', first_layer_tensor)
  ...
  return (train_step, cross_entropy_mean, bottleneck_input, ground_truth_input,
          final_tensor, semi_final_tensor, semi_layer_biases, first_layer_tensor)
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    (train_step, cross_entropy, bottleneck_input, ground_truth_input,
     final_tensor, semi_final_tensor, semi_layer_biases, first_layer_tensor) = add_final_training_ops(
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        if i==0:
            best_idx = i 
            best_train = cross_entropy_value
            best_train_acc = train_accuracy
            best_val = val_cross_entropy
            best_val_acc = validation_accuracy
        else:
            if val_cross_entropy<best_val and validation_accuracy>=best_val_acc:
                print('------------------------------------------------------------------')
                best_idx = i 
                best_train = cross_entropy_value
                best_train_acc = train_accuracy
                best_val = val_cross_entropy
                best_val_acc = validation_accuracy
                print('New best step {0}, train cross entropy = {1}, train acc = {2}%'.format(best_idx, best_train, best_train_acc*100))
                print('New best step {0}, val cross entropy = {1}, val acc = {2}%'.format(best_idx, best_val, best_val_acc*100))
    ...
    if 1:
      tf.logging.info('======== F1 SCORE ========')
      TP, FP, TN, FN = 0, 0, 0, 0
      
      for i, test_filename in enumerate(test_filenames):
        if list(image_lists.keys())[predictions[i]]=='1':
      ...
      try:  
          ...
          R = TP/(TP+FN)
          F1 = (2*P*R)/(P+R)
          F1_acc = (TP + TN)/(TP+TN+FP+FN)
      except ZeroDivisionError:
          P = 0
          R = 0
          F1 = 0
      print(TP, FP, TN, FN, F1_acc)
      print("PRECISION \t{0}".format(P))
      print("RECALL \t\t{0}".format(R))
      print("F1 \t\t{0}".format(F1))
      print('==================================================================')
      
    tf.logging.info('Best step: Step %d, Train cross entropy = %f, Train acc = %.1f%%' %
                    (best_idx, best_train, best_train_acc * 100))
    tf.logging.info('Best step: Step %d, Val cross entropy = %f, Val acc = %.1f%%' %
                    (best_idx, best_val, best_val_acc * 100))
