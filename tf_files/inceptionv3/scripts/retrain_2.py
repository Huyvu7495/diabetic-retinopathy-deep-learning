...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            ...
            semi_layer_weights = tf.Variable(
                semi_initial_value, name='semi_final_weights')
        ...
        with tf.name_scope('biases'):
            semi_layer_biases = tf.Variable(
                tf.zeros([128]), name='semi_final_biases')
        ...
        with tf.name_scope('Wx_plus_b'):
            semi_logits = tf.matmul(
                bottleneck_input, semi_layer_weights) + semi_layer_biases
    ...
    with tf.name_scope('train'):
        ...
        optimizer = tf.train.MomentumOptimizer(
            FLAGS.learning_rate, momentum=0.9, use_nesterov=True)
            ...
            if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
                ...
                if i == 0:
                    ...
                    if val_cross_entropy < best_val and validation_accuracy >= best_val_acc:
                        ...
                        best_val = val_cross_entropy
                        best_val_acc = validation_accuracy
                        print('New best step {0}, train cross entropy = {1}, train acc = {2}%'.format(
                            best_idx, best_train, best_train_acc * 100))
                        print('New best step {0}, val cross entropy = {1}, val acc = {2}%'.format(
                            best_idx, best_val, best_val_acc * 100))
                        save_graph_to_file(
                            sess, graph, FLAGS.output_graph.replace('.pb', '_best.pb'))
